...
export default function MiNutricionista() {
  ...
  useEffect(() => {
    getProfesionales('nutricionista')
      .then(({ data }) => setProfesionales(data))
      .catch(() => setError('No se pudieron cargar los nutricionistas'))
  ...
  const handleSolicitar = async (profesional) => {
    ...
    setError('')
    try {
      await solicitarVinculo(profesional.id_usuario, 'nutricionista')
  ...
  return (
    <AppLayout>
      <div className="min-h-screen px-6 pt-16 pb-8" style={{ backgroundColor: '#000', maxWidth: '720px' }}>
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold" style={{ color: '#fff' }}>Mi nutricionista</h1>
          <p className="text-sm mt-1" style={{ color: '#333' }}>
            Encontrá un nutricionista y enviá una solicitud
          </p>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-center py-12" style={{ color: '#444' }}>No se encontraron nutricionistas</p>
        ) : (
