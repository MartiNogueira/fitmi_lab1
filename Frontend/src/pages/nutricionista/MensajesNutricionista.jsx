...
const conversations = [
  { id: 1, initials: 'MR', name: 'María Rodriguez', lastMessage: '¿Puedo reemplazar la avena por granola?', time: 'hace 5 min', unread: 1,
    messages: [
      { from: 'paciente', text: 'Hola! Seguí el plan esta semana y bajé 400g', time: '09:00' },
      { from: 'nutricionista', text: 'Excelente progreso, seguí así!', time: '09:05' },
      { from: 'paciente', text: '¿Puedo reemplazar la avena por granola?', time: '09:30' },
    ]},
  { id: 2, initials: 'LP', name: 'Lucas Perez', lastMessage: 'Gracias por el plan nuevo!', time: 'hace 2 horas', unread: 0,
    messages: [
      { from: 'nutricionista', text: 'Lucas, te actualicé el plan para esta semana', time: '08:00' },
      { from: 'paciente', text: 'Gracias por el plan nuevo!', time: '08:20' },
    ]},
  { id: 3, initials: 'TC', name: 'Tomás Castro', lastMessage: 'Tuve un pico de glucemia ayer', time: 'ayer', unread: 2,
    messages: [
      { from: 'paciente', text: 'Tuve un pico de glucemia ayer', time: 'ayer 19:00' },
      { from: 'paciente', text: 'Comí algo fuera del plan, me disculpo', time: 'ayer 19:01' },
    ]},
]
...
export default function MensajesNutricionista() {
  ...
  const send = () => {
    ...
    setAllConvs((prev) => prev.map((c) => c.id === activeId
      ? { ...c, messages: [...c.messages, { from: 'nutricionista', text: input.trim(), time: 'ahora' }], lastMessage: input.trim(), time: 'ahora', unread: 0 }
      : c))
    setInput('')
  }
        ...
        <div className="flex-1 flex flex-col" style={{ minWidth: 0 }}>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3">
            {active?.messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.from === 'nutricionista' ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-xs px-4 py-2 rounded-2xl text-sm"
                  style={msg.from === 'nutricionista'
